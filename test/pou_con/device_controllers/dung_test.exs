defmodule PouCon.DeviceControllers.DungControllerTest do
  use PouCon.DataCase
  import Mox
  ...
  setup do
    ...
    device_names = %{
      on_off_coil: "test_dung_coil_#{id}",
      running_feedback: "test_dung_fb_#{id}"
    }
  ...
  describe "start/1 and initialization" do
    test "starts successfully with valid options", %{devices: devices} do
      opts = [
        name: "test_dung_1_#{System.unique_integer([:positive])}",
        title: "Test Dung 1",
        on_off_coil: devices.on_off_coil,
        running_feedback: devices.running_feedback
      ]

      assert {:ok, pid} = DungController.start(opts)
  ...
  describe "status/1" do
    setup %{devices: devices} do
      name = "test_dung_status_#{System.unique_integer([:positive])}"

      opts = [
        name: name,
        title: "Test Status Dung",
        on_off_coil: devices.on_off_coil,
        running_feedback: devices.running_feedback
      ]

      {:ok, _pid} = DungController.start(opts)
      %{name: name}
    end

    test "returns status map with all required fields", %{name: name} do
      status = DungController.status(name)
    ...
    test "reflects state from DeviceManager", %{devices: devices} do
      name = "test_dung_state_#{System.unique_integer([:positive])}"
  ...
  describe "commands" do
    setup %{devices: devices} do
      name = "test_dung_cmd_#{System.unique_integer([:positive])}"
      ...
      {:ok, pid} = DungController.start(opts)
      %{name: name, pid: pid}
    end
