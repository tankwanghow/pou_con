defmodule PouCon.Equipment.Controllers.DungExitTest do
  ...
  setup do
    ...
    device_names = %{
      on_off_coil: "test_dungexit_coil_#{id}",
  ...
  describe "start/1 and initialization" do
    test "starts successfully with valid options", %{devices: devices} do
      opts = [
        name: "test_dungexit_1_#{System.unique_integer([:positive])}",
        title: "Test DungExit 1",
  ...
  describe "status/1" do
    setup %{devices: devices} do
      name = "test_dungexit_status_#{System.unique_integer([:positive])}"

      opts = [
        name: name,
        title: "Test Status DungExit",
        on_off_coil: devices.on_off_coil,
        running_feedback: devices.running_feedback
      ]
  ...
  describe "commands" do
    setup %{devices: devices} do
      name = "test_dungexit_cmd_#{System.unique_integer([:positive])}"
