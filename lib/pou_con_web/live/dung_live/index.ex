defmodule PouConWeb.DungLive do
      ...
      <div class="p-4">
        ...
        <div class="flex flex-wrap gap-1 mb-6">
          <%= for eq <- Enum.filter(@equipment, &(&1.type == "dung")) |> Enum.sort_by(& &1.title) do %>
            <.live_component module={PouConWeb.Components.DungComponent} id={eq.name} equipment={eq} />
          <% end %>
          <%= for eq <- Enum.filter(@equipment, &(&1.type == "dung_horz")) |> Enum.sort_by(& &1.title) do %>
            <.live_component
              module={PouConWeb.Components.DungHorComponent}
              id={eq.name}
              equipment={eq}
            />
          <% end %>
          <%= for eq <- Enum.filter(@equipment, &(&1.type == "dung_exit")) |> Enum.sort_by(& &1.title) do %>
            <.live_component
              module={PouConWeb.Components.DungExitComponent}
              id={eq.name}
              equipment={eq}
            />
          <% end %>
