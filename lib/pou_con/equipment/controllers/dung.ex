defmodule PouCon.Equipment.Controllers.Dung do
  ...
  # Default polling interval for dung conveyors (500ms for responsive feedback)
  ...
  defp sync_coil(%State{commanded_on: cmd, actual_on: act, on_off_coil: coil, inverted: inv} = state)
       when cmd != act do
    Logger.info("[#{state.name}] #{if cmd, do: "Turning ON", else: "Turning OFF"} dung")
  ...
  defp poll_and_update(nil) do
    Logger.error("Dung: poll_and_update called with nil state — recovering")
