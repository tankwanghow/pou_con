defmodule PouCon.DeviceControllers.DungController do
  use GenServer
  require Logger
  ...
  defp sync_coil(%State{commanded_on: cmd, actual_on: act, on_off_coil: coil} = state)
       when cmd != act do
    Logger.info("[#{state.name}] #{if cmd, do: "Turning ON", else: "Turning OFF"} dung pump")
  ...
  defp sync_and_update(nil) do
    Logger.error("DungController: sync_and_update called with nil state — recovering")
