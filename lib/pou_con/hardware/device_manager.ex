defmodule PouCon.Hardware.DeviceManager do
  @moduledoc """
  ...
  **No business logic. No direction selection. No pulse timeouts.**
  ...
  @behaviour PouCon.Hardware.DeviceManagerBehaviour
  ...
  def handle_call({:simulate_offline, device_name, offline?}, _from, state) do
    case get_device_and_modbus(state, device_name) do
      {:ok, dev, modbus} when modbus != nil ->
        PouCon.Hardware.Modbus.SimulatedAdapter.set_offline(modbus, dev.slave_id, offline?)
        {:reply, :ok, state}
    ...
    case get_device_and_modbus(state, device_name) do
      {:ok, dev, modbus} when modbus != nil ->
        ...
        if values[:temperature] do
          ...
          PouCon.Hardware.Modbus.SimulatedAdapter.set_register(modbus, dev.slave_id, base, val)
        end
  ...
  def handle_cast({:remove_port, device_path}, state) do
    case Map.get(state.ports, device_path) do
      ...
      port ->
        if port.modbus_pid, do: PouCon.Hardware.PortSupervisor.stop_modbus_master(port.modbus_pid)
    ...
    runtime_ports =
      ...
      |> Enum.reduce(%{}, fn db_port, acc ->
        if db_port.device_path == "virtual" do
          ...
          case PouCon.Hardware.PortSupervisor.start_modbus_master(db_port) do
            {:ok, pid} ->
              Map.put(acc, db_port.device_path, %RuntimePort{
                device_path: db_port.device_path,
                modbus_pid: pid,
